...
if (empty($id_aluno) || empty($id_professor) || empty($id_arena) || empty($dias_aula) || empty($horario_ini) || empty($horario_fim)) {
...
try {
    // Prepara o INSERT
    $sql = "INSERT INTO info_geral.aulas_particulares (id_aluno, id_professor, id_arena, dias_aula, horario_inicio, horario_fim)
    ...
    echo "<script>alert('Erro ao cadastrar aula: " . $e->getMessage() . "'); history.back();</script>";
    exit;
}
